...
| New feature planning backlog | Written | Added `docs/NEW_FEATURE_PLANNING.md` to break the next commercial feature work into validation, Buddy Browser, Buddy Policy, Buddy Audit, Buddy Agent loop, Buddy Bridge, Adapter Center, Browser Workbench, PaaS Bundle lifecycle, and commercial packaging epics. |
...
| PaaS cached bundle selector | Written | PaaS controls can select any cached Bundle entry, show its metrics/details, and preview/install the selected cache item instead of only the newest one. |
| PaaS controls dock | Written | Added `PaaSControlsDock.tsx` to save/clear sessions, register device, pull bundles, push sync outbox, show bundle cache/schema state, choose cached bundles, and preview/install selected cached bundles with explicit target controls. |
...
5. Use `NEW_FEATURE_PLANNING.md` as the executable backlog for the next commercial feature pass.
