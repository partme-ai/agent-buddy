...
export interface ConsoleSyncSummary {
  pending: number
  failed: number
  sent: number
  skipped: number
  flushPlan: SyncFlushPlan
}
...
export interface ConsoleOverviewDashboard {
  generatedAt: number
  metrics: ConsoleMetric[]
  healthScore: number
  runtimeSummary: ConsoleRuntimeSummary
  syncSummary: ConsoleSyncSummary
  recentEvents: ConsoleTimelineEvent[]
  warnings: string[]
}

export interface ConsoleRuntimeCheck {
  runtime: RuntimeKind
  label: string
  status: string
  path?: string | null
...
export interface ConsoleRiskAlert {
  id: string
...
export interface ConsoleHealthBoard {
  generatedAt: number
  doctor: DoctorReport
  runtimeChecks: ConsoleRuntimeCheck[]
  riskAlerts: ConsoleRiskAlert[]
  recentTasks: ConsoleTimelineEvent[]
  syncFailures: ConsoleTimelineEvent[]
  warnings: string[]
}
...
export interface ConsoleInstanceGroup {
  id: string
  label: string
  count: number
  tags: string[]
...
export interface CleanupPreviewContext {
