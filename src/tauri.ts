...
export function createKnowledgeSnapshot(spaceId: string, version: string, manifestPath: string): Promise<KnowledgeSnapshot> {
  return invoke('create_knowledge_snapshot', { spaceId, version, manifestPath })
}
...
export function proposeMemory(content: string, scope: MemoryScope, memoryType: MemoryType, sourceSessionId?: string | null): Promise<MemoryCandidate> {
  return invoke('propose_memory', { content, scope, memoryType, sourceSessionId: sourceSessionId ?? null })
}
export function approveMemoryCandidate(candidateId: string, title: string): Promise<MemoryItem> {
  return invoke('approve_memory_candidate', { candidateId, title })
}

export function appendSessionEvent(sessionId: string, runtime: RuntimeKind | null, eventType: SessionEventType, payloadJson: string): Promise<SessionEvent> {
  return invoke('append_session_event', { sessionId, runtime, eventType, payloadJson })
}
...
export function createHandoffPack(sessionId: string, fromRuntime: RuntimeKind | null, toRuntime: RuntimeKind | null, goal: string, summary: string): Promise<HandoffPack> {
  return invoke('create_handoff_pack', { sessionId, fromRuntime, toRuntime, goal, summary })
}
