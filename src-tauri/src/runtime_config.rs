...
pub struct RuntimeConfigPreview { pub runtime: RuntimeKind, pub files: Vec<RuntimeConfigFile>, pub warnings: Vec<String> }

#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct RuntimeConfigFile { pub relative_path: String, pub content: String, pub merge_strategy: String }
...
pub fn mcp_config_preview(runtime: RuntimeKind, servers: &[McpServerConfig]) -> RuntimeConfigPreview {
    match runtime {
        ...
        _ => RuntimeConfigPreview { runtime, files: Vec::new(), warnings: vec!["This runtime does not have a known MCP configuration format yet; use Buddy local HTTP API or manual MCP setup.".to_string()] },
    }
}
...
fn json_config(runtime: RuntimeKind, relative_path: &str, servers: &[McpServerConfig]) -> RuntimeConfigPreview {
    ...
    RuntimeConfigPreview { runtime, files: vec![RuntimeConfigFile { relative_path: relative_path.to_string(), content: serde_json::to_string_pretty(&value).unwrap_or_else(|_| "{}".to_string()), merge_strategy: "merge-agent-buddy-managed-mcp-section".to_string() }], warnings: Vec::new() }
}
...
fn codex_toml_config(servers: &[McpServerConfig]) -> RuntimeConfigPreview {
    ...
    for server in servers.iter().filter(|server| server.enabled) {
        ...
        match &server.url {
            Some(url) => content.push_str(&format!("url = \"{}\"\n", escape_toml(url))),
            None => {
...
fn sanitize_toml_key(value: &str) -> String { value.chars().map(|ch| if ch.is_alphanumeric() || ch == '_' { ch } else { '_' }).collect() }
