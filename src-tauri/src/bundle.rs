...
pub fn bundle_from_local_agent(agent: &LocalAgent, targets: Vec<RuntimeKind>) -> AgentBundle {
    AgentBundle {
        bundle_id: format!("agency-agents-zh.{}", agent.slug),
        ...
        memory: MemoryPolicy {
            provider: "agent-buddy".to_string(),
            read_scopes: vec!["user".to_string(), "agent".to_string(), "project".to_string()],
            write_policy: "approval_required".to_string(),
        },
        skills: vec![SkillRef { id: agent.slug.clone(), source: "agency-agents-zh".to_string(), version: None }],
        mcp_servers: default_buddy_mcp_refs(),
        permissions: PermissionPolicy {
            file_write: "ask".to_string(),
            network: "ask".to_string(),
            shell: "ask".to_string(),
            external_publish: "approval_required".to_string(),
        },
        targets,
        source: BundleSource {
...
pub fn default_buddy_mcp_refs() -> Vec<McpServerRef> {
    vec![
        McpServerRef { id: "buddy-memory".to_string(), required: false },
        McpServerRef { id: "buddy-knowledge".to_string(), required: false },
        McpServerRef { id: "buddy-session".to_string(), required: false },
        McpServerRef { id: "buddy-approval".to_string(), required: false },
    ]
}
