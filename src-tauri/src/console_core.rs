...
pub struct ConsoleOverviewDashboard {
    pub generated_at: i64,
    pub metrics: Vec<ConsoleMetric>,
    pub health_score: i64,
    pub runtime_summary: ConsoleRuntimeSummary,
    pub sync_summary: ConsoleSyncSummary,
    pub recent_events: Vec<ConsoleTimelineEvent>,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct ConsoleHealthBoard {
    pub generated_at: i64,
    pub doctor: DoctorReport,
    pub runtime_checks: Vec<ConsoleRuntimeCheck>,
    pub risk_alerts: Vec<ConsoleRiskAlert>,
    pub recent_tasks: Vec<ConsoleTimelineEvent>,
    pub sync_failures: Vec<ConsoleTimelineEvent>,
    pub warnings: Vec<String>,
}
...
pub fn build_overview_dashboard(settings: &AgentBuddySettings, runtimes: Vec<RuntimeDetection>, installations: Vec<AgentInstallation>, agents_count: usize, session_events: Vec<SessionEvent>, sync_events: Vec<SyncOutboxEvent>, audit_events: Vec<AuditEvent>) -> ConsoleOverviewDashboard {
    ...
    ConsoleOverviewDashboard {
        ...
        metrics: vec![
            metric("instances", "实例数", (runtime_summary.total + installations.len()) as i64, "count"),
            metric("agents", "总智能体数", agents_count as i64, "count"),
            metric("installations", "已安装", installations.len() as i64, "count"),
            metric("sessions", "活跃会话", active_sessions, "count"),
            metric("pendingSync", "待同步", sync_summary.pending as i64 + sync_summary.failed as i64, "count"),
            metric("risks", "风险告警", risk_count, "count"),
        ],
        health_score,
        runtime_summary,
        sync_summary,
...
pub fn build_console_instances(runtimes: Vec<RuntimeDetection>, installations: Vec<AgentInstallation>, mcp_servers: Vec<McpServerConfig>, knowledge_spaces: Vec<KnowledgeSpace>, memory_items: Vec<MemoryItem>, memory_candidates: Vec<MemoryCandidate>, session_events: Vec<SessionEvent>, local_api: Option<LocalApiSpec>) -> Vec<ConsoleInstance> {
    ...
    for server in mcp_servers { instances.push(ConsoleInstance { id: format!("mcp:{}", server.id), name: server.name, instance_type: "mcp-service".to_string(), status: if server.enabled { "running".to_string() } else { "stopped".to_string() }, group: "MCP Services".to_string(), runtime: None, path: server.url.or(server.command), tags: vec![format!("{:?}", server.transport), if server.policy.approval_required { "approval".to_string() } else { "auto".to_string() }], description: server.description, updated_at: None }); }
...
pub fn build_instance_groups(instances: &[ConsoleInstance]) -> Vec<ConsoleInstanceGroup> {
    ...
    for instance in instances {
        let group = groups.entry(instance.group.clone()).or_insert_with(|| ConsoleInstanceGroup { id: instance.group.to_lowercase().replace(' ', "-"), label: instance.group.clone(), count: 0, tags: Vec::new(), status_counts: BTreeMap::new() });
        group.count += 1;
        *group.status_counts.entry(instance.status.clone()).or_insert(0) += 1;
        for tag in &instance.tags { if !group.tags.contains(tag) { group.tags.push(tag.clone()); } }
    }
...
pub fn build_local_daemon_plan(api: LocalApiSpec, mcp_servers: Vec<McpServerConfig>) -> LocalDaemonPlan {
    let enabled_mcp = mcp_servers.iter().filter(|server| server.enabled).count();
    let mut warnings = Vec::new();
    warnings.push("Local daemon is currently a preview plan; route spec exists but the HTTP/MCP server is not started yet.".to_string());
    LocalDaemonPlan { enabled: false, bind_host: api.bind_host, bind_port: api.bind_port, base_url: api.base_url, route_count: api.routes.len(), mcp_server_count: enabled_mcp, capabilities: vec!["memory".to_string(), "knowledge".to_string(), "session".to_string(), "approval".to_string(), "mcp-proxy".to_string()], warnings }
}

fn runtime_summary(runtimes: &[RuntimeDetection]) -> ConsoleRuntimeSummary {
    let mut by_scope = BTreeMap::new();
    for runtime in runtimes { *by_scope.entry(format!("{:?}", runtime.scope)).or_insert(0) += 1; }
    ConsoleRuntimeSummary { total: runtimes.len(), detected: runtimes.iter().filter(|runtime| runtime.detected).count(), not_detected: runtimes.iter().filter(|runtime| !runtime.detected).count(), by_scope }
}

fn sync_summary(events: Vec<SyncOutboxEvent>, flush_plan: SyncFlushPlan) -> ConsoleSyncSummary {
...
fn health_score(detected: usize, total: usize, risk_count: i64, failed_sync: i64) -> i64 {
    if total == 0 { return 0; }
    let base = ((detected as f64 / total as f64) * 100.0).round() as i64;
    (base - risk_count * 2 - failed_sync * 3).clamp(0, 100)
}
