...
fn refresh_agent_source(state: State<'_, AppState>) -> Result<SourceRefreshResult, String> {
    ...
    state.db.save_audit_event(&audit_event("source.refresh", "agent_source", &result.source_id, None, AuditSeverity::Info, "refreshed agency-agents-zh source")).map_err(to_message)?;
    Ok(result)
}
...
fn detect_runtimes(state: State<'_, AppState>) -> Result<Vec<RuntimeDetection>, String> {
    ...
    for detection in &detections { state.db.save_runtime_detection(detection).map_err(to_message)?; }
    Ok(detections)
}
...
pub fn run() {
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            load_settings, save_settings, refresh_agent_source, list_agents, build_agent_bundles,
            detect_runtimes, runtime_definitions, get_install_plan, install_agents,
            ...
            create_knowledge_snapshot, list_memory_items, list_memory_candidates, propose_memory,
            approve_memory_candidate, append_session_event, list_session_events, list_handoff_packs,
            create_handoff_pack, restore_backup, uninstall_installation, run_doctor, parse_deeplink
        ])
